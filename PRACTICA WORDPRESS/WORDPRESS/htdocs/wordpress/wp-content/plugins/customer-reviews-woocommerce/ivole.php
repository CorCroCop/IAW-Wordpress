...
function ivole_email_unsubscribe_shortcode() {
	...
	if( isset( $_POST['ivole_submit'] ) && isset( $_POST['ivole_email'] ) ) {
		if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
			...
			if( !in_array( $email, $opt_out_emails ) ) {
				...
				echo '<p>' . __('Success: you have unsubscribed from emails related to reviews!', 'customer-reviews-woocommerce') . '</p>';
			} else {
				echo '<p>' . __('Success: you have unsubscribed from emails related to reviews!', 'customer-reviews-woocommerce') . '</p>';
			}
		} else {
			echo '<p>' . __('Error: please provide a valid email address!', 'customer-reviews-woocommerce') . '</p>';
		}
		echo '<a href="' . get_home_url() . '">' . __( 'Go to home page', 'customer-reviews-woocommerce' ) . '</a>';
		return;
	}
	?>
	<div class="ivole-unsubscribe-form">
		<form action="" method="post">
			...
			<p>
				<label for="ivole_email"><?php _e('Email Address:', 'customer-reviews-woocommerce'); ?></label>
				<input type="text" id="ivole_email" name="ivole_email" value="<?php echo esc_attr($email); ?>" size="25" />
			</p>
			<p>
				<input type="submit" name="ivole_submit" value="<?php _e('Unsubscribe', 'customer-reviews-woocommerce'); ?>" />
			</p>
		</form>
	</div>
	<?php
