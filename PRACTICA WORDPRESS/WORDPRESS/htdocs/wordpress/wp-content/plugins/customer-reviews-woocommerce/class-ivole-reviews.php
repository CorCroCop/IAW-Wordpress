				...
				if( $nFiles > 0 ) {
					if( $nFiles > $this->limit_file_count ) {
						echo __( "Error: You tried to upload too many files. The maximum number of files that you can upload is " .
							$this->limit_file_count . ".<br/> Go back to: ", 'customer-reviews-woocommerce' );
					...
					for( $i = 0; $i < $nFiles; $i++ ) {
						...
						if ( $this->limit_file_size < $_FILES[ $comment_image_id ]['size'][$i] ) {
							echo __( "Error: Uploaded file is too large. <br/> Go back to: ", 'customer-reviews-woocommerce' );
							echo '<a href="' . get_permalink( $post_id ) . '">' . get_the_title( $post_id ) . '</a>';
							die;
						}
						// Get file extension
			...
			if( count( $comments ) > 0 ) {
				...
				foreach( $comments as $comment ) {
					...
					if( $pics_n > 0 || $pics_v_n > 0 ) {
						...
						if( $pics_v_n > 0 ) {
							$comment->comment_content .= '<p class="iv-comment-video-text">' . __( 'Uploaded video(s):', 'customer-reviews-woocommerce' ) . '</p>';
